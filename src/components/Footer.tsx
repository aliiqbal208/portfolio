...
// Icon mapping from string to component with styling
const SOCIAL_ICON_MAP: Record<string, { icon: () => React.ReactNode; color: string; bg: string }> = {
  linkedin:  { icon: LinkedinIcon,  color: "hover:text-[#0a66c2]", bg: "group-hover:bg-[#0a66c2]/15" },
  github:    { icon: GithubIcon,    color: "hover:text-white",     bg: "group-hover:bg-white/10" },
  x:         { icon: XIcon,         color: "hover:text-white",     bg: "group-hover:bg-white/10" },
  medium:    { icon: MediumIcon,    color: "hover:text-white",     bg: "group-hover:bg-white/10" },
  instagram: { icon: InstagramIcon, color: "hover:text-[#e1306c]", bg: "group-hover:bg-[#e1306c]/15" },
  discord:   { icon: DiscordIcon,   color: "hover:text-[#5865f2]", bg: "group-hover:bg-[#5865f2]/15" },
};

function SocialLink({ social }: { social: SocialItem }) {
  const config = SOCIAL_ICON_MAP[social.icon] || SOCIAL_ICON_MAP.github;
  ...
  return (
    <a
      ...
      className={`group relative p-3 text-muted-foreground ${config.color} transition-all duration-300`}
    >
      <div className={`absolute inset-0 scale-0 rounded-full ${config.bg} group-hover:scale-100 transition-transform duration-300`} />
    ...
    <footer className="relative py-12 px-8 border-t border-white/5 bg-background">
      <div className="max-w-7xl mx-auto">
        <div className="flex flex-col md:flex-row justify-between items-center gap-12">
          <div className="flex flex-col gap-2 text-center md:text-left">
            <div className="flex items-center gap-2 justify-center md:justify-start">
              <span className="text-2xl font-bebas text-white tracking-widest">
                {footer.name}<span className="text-primary">.</span>
              </span>
            </div>
          ...
          >
            <span>{footer.topButtonLabel}</span>
