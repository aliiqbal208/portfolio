...
function OrbitalCard({
  ...
  data: typeof TESTIMONIALS[0]; index: number; total: number; radiusX: number; radiusY: number; rotation: ReturnType<typeof useSpring>;
}) {
  ...
  return (
    ...
    >
      ...
      {isMobile ? (
        ...
        >
          {TESTIMONIALS.map((t, i) => (
            <OrbitalCard key={i} data={t} index={i} total={TESTIMONIALS.length} radiusX={radiusX} radiusY={radiusY} rotation={rotation} />
          ))}
        </motion.div>
      )}
    </section>
