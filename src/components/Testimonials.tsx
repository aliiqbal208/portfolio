  ...
  return (
    <>
      <div className="relative z-10 flex items-center justify-between mb-3 md:mb-4">
        ...
        <Icon className="w-5 h-5 md:w-6 md:h-6 text-primary/80 group-hover:text-primary transition-colors" />
      </div>
      <div className="relative z-10 bg-navy/50 backdrop-blur-sm rounded-xl p-3 md:p-4 border border-white/5 group-hover:border-primary/20 transition-colors">
...
function OrbitalCard({
  ...
  data: TestimonialItem; index: number; total: number; radiusX: number; radiusY: number; rotation: ReturnType<typeof useSpring>;
}) {
...
export default function Testimonials() {
  ...
  useEffect(() => {
    const update = () => {
      ...
      setIsMobile(mobile);
      if (!mobile) {
        setRadiusX(w >= 1280 ? 480 : 320);
        setRadiusY(w >= 1280 ? 220 : 170);
      }
    };
    update();
  ...
  return (
    <section
      className={`relative bg-navy overflow-hidden flex flex-col items-center justify-center py-16 md:py-20 ${isMobile ? "gap-8" : "min-h-[150vh]"}`}
      id="testimonials"
    >
      ...
      {!isMobile && (
        <div className="absolute inset-0 flex items-center justify-center pointer-events-none opacity-30">
          <svg className="w-full h-full max-w-[1200px] max-h-[1200px]" viewBox="0 0 1000 1000">
            ...
            <ellipse cx="500" cy="500" rx={Math.max(0, radiusX - 80)} ry={Math.max(0, radiusY - 80)} fill="none" stroke="var(--color-primary)" strokeWidth="0.5" strokeDasharray="8 2" />
          </svg>
        </div>
      )}

      {/* Center heading */}
      <div className={`relative z-10 text-center ${isMobile ? "" : "pointer-events-none"}`}>
        <motion.h1
        ...
        >
          {testimonials.heading.line1}<br /><span className="z-0"> {testimonials.heading.line2}</span>
        </motion.h1>
      </div>

      {isMobile ? (
        /* Mobile: scrollable vertical stack */
        <div className="relative z-10 w-full flex flex-col gap-4 px-4">
          {testimonials.items.map((t, i) => (
            <motion.div
              ...
              className="group glass-dark rounded-2xl p-4 overflow-hidden transition-all duration-500 hover:border-primary/50 hover:shadow-[0_0_40px_rgba(8,136,174,0.2)]"
            >
              <div className="absolute inset-0 bg-linear-to-br from-primary/10 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500 pointer-events-none rounded-2xl" />
              <CardContent data={t} />
            </motion.div>
          ))}
        </div>
